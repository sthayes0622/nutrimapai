          ...
          {isPremium ? (
            ...
            <div className="space-y-4">
              <div className="text-sm text-gray-600">
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(account.usageThisMonth * 100, 100)}%` }}
                  />
                </div>
              </div>
              ...
              <div className="border border-green-200 rounded-xl p-4 bg-green-50">
                ...
                <ul className="text-sm text-gray-600 space-y-1 mb-4">
                  {["Unlimited meal plans", "Unlimited grocery lists", "Priority AI generation"].map((f) => (
                    <li key={f} className="flex items-center gap-2">
                      <CheckIcon className="w-4 h-4 text-green-600" /> {f}
                    </li>
                  ))}
                </ul>
                <div className="flex gap-3">
                  <button
                    onClick={() => handleUpgrade(process.env.NEXT_PUBLIC_STRIPE_PREMIUM_MONTHLY_PRICE_ID!)}
                    disabled={loading}
                    className="flex-1 bg-green-600 text-white py-2.5 rounded-lg text-sm font-semibold hover:bg-green-700 transition-colors disabled:opacity-50"
                  >
                    $7.99/month
                  </button>
                  <button
                    onClick={() => handleUpgrade(process.env.NEXT_PUBLIC_STRIPE_PREMIUM_YEARLY_PRICE_ID!)}
                    disabled={loading}
                ...
                {loading && <p className="text-xs text-center text-gray-500 mt-2">Redirecting to checkout...</p>}
              </div>
            </div>
          )}
        </div>
